...
int main(int /*argc*/, char** /*argv*/) {
    try {
        ...
        cv::Mat oFrame;
        oSequence >> oFrame;
        CV_Assert(!oFrame.empty() && oFrame.type()==CV_8UC1 && oFrame.size()==cv::Size(256,256));
        ...
        constexpr int nBlockSize = 16; // let's use big blocks, it will be easier to track large patterns!
        ...
        cv::rectangle(oFrame_display_base,oDefaultBlockRegion,cv::Scalar_<uchar>(255),2); // highlight base block location in white
        bool bFirstFrame = true;
        double dCurrEQM;
        while(!oFrame.empty()) {

            const cv::Rect oDefaultSearchRegion(150,100,60,60); // some hard-coded search region... (you should try more than one region! borders too!)
            const cv::Point2i vBlockOffsetVec = tp2::match_block(oDefaultBlock,oDefaultBlockPos,oDefaultSearchRegion,oFrame,dCurrEQM);
            ...
            if(bFirstFrame) {
                ...
                CV_Assert(vBlockOffsetVec.x==0 && vBlockOffsetVec.y==0); // this is only valid for the hard-coded values above; remove this if you intend on running more complicated tests later
            ...
            const cv::Rect oNextBlockRegion(oDefaultBlockPos+vBlockOffsetVec,cv::Size(nBlockSize,nBlockSize));
            cv::rectangle(oFrame_display_next,oNextBlockRegion,cv::Scalar_<uchar>(255),2); // highlight predicted block location in white
            cv::rectangle(oFrame_display_next,oDefaultSearchRegion,cv::Scalar_<uchar>(128),1); // highlight search zone location in gray
