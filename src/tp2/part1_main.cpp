...
int main(int /*argc*/, char** /*argv*/) {
    try {
        ...
        for(const tp2::ImagePathFlag& oImagePathFlag : vsTestImages) { // for each test image
            const cv::Mat oInput = cv::imread(oImagePathFlag.first,oImagePathFlag.second); // load the test image
            if(oInput.empty()) {
                std::cerr << "Could not load image at '" << oImagePathFlag.first << "'; exiting..." << std::endl;
                std::exit(-1);
            }
            ...
            // note: matrices of the form cv::Mat_<X> can only have element type 'X' assigned at each pixel;
            // you can access this element directly via 'mat(row,col)' instead of mat.at<X>(row,col)'
            ...
            // e.g. to test if conv_rgb2ycbcr and conv_ycbcr2rgb work properly, try decompressing here:
            ...
            const std::vector<std::array<short,8*8>> vInlinedBlocks_decompr = tp2::huff::decode(oCode);
            ...
            for(size_t b=0; b<vQuantifDCTBlocks_decompr.size(); ++b)
                vDCTBlocks_decompr[b] = tp2::quantif_inv(vQuantifDCTBlocks_decompr[b],USE_QUANT_QUALITY);
            ...
            if(oInput_decompr.size()!=oInput.size()) {
                std::cout << "ERROR! Reconstructed image had different size from original... aborting." << std::endl;
                std::exit(-1);
            }
            cv::Mat oDisplay;
            ...
            const std::string sWinName = oImagePathFlag.first.substr(oImagePathFlag.first.find_last_of("/\\")+1)+" orig/reconstr/diff";
            ...
            if(tp2::isEqual<uint8_t>(oInput,oInput_decompr))
                ...
                std::cout << "\t... done. Compression is LOSSY (PSNR=" << dPSNR << "dB, and " << float(cv::countNonZero(oInput!=oInput_decompr))*100/oInput.total() << "% of pixels have altered values)." << std::endl;
            }
        }
